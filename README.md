...
- **Prospective (anticipatory) connectivity**: `build_torus_connectivity` shifts each presynaptic cell's effective toroidal phase by a fixed distance `l_asym` along that cell's own preferred head direction before computing its connection weight to each omnidirectional cell. This offset — not the presynaptic firing itself — is what makes the omnidirectional population's firing fields prospective, and is the mechanism the inclination-induced shift (below) acts through.
