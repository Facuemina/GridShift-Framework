...
![Inclination-induced shift mechanism](scheme.png)
...
3. Produces figures (`Figures/` subfolder, saved as ``):
   - `CrossCorrelograms_zoom` — mean cross-correlograms per session
   - `SpatialMaps` — trajectory overlays, rate maps, and single-cell autocorrelograms
   - `Shifts_Y` — boxplots of the X/Y grid-pattern shift across sessions
   - `Mean_fr` — mean firing rate per session
   - `UP_DOWN-Correlation` — running-direction (up vs. down) shift comparison and population spatial-correlation analysis
