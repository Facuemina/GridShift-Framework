...
def run_job(path2save, l_asym, l_torus, inclination_angle, 
            ...
            thresh_weights=[0.3,0.3,0.3,0],T0=80,T1=10,VISUAL='ON'):
    ...
    cmd = [
        ...
        f"--T0={T0}",f"--T1={T1}",f"--VISUAL={VISUAL}"
    ]
        ...
        for l_torus in periodicities:
            for l_asym in asymmetries:
                for i_incl, incl_ang in tqdm(enumerate(inclination_angs)):
                    ...
                    pars['seed'] = i_incl + 55 * (num-10)
                    pars['l_torus'] = l_torus
                    pars['thresh'] = 0#[0,
                                      #0, 
                                      #0]#*(45  + 12 * pars['A_vest'] * np.sin(incl_ang))]#300 * (np.cos(incl_ang) + 1)]
                    ...
                    # pars['gain'] = [1,1,.1]
                    # pars['k'] = [1e-3, 5 * 1e-3, 0*1e-4]
                    
                    # pars['gain'] = [1,3,10]#.03]
                    # pars['k'] = [1e-3, 5 * 1e-3, 1]#1e-3]
